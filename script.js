...
const reviews=[['A L','AL','Absolut empfehlenswert! Hervorragender Service, kompetente Beratung und ein tolles Ergebnis. Man fühlt sich vom ersten Moment an wohl. Vielen Dank an das ganze Team!'],['Mario Vorfaj','MV','Der Laden ist sehr sauber, die Mitarbeiter sind pünktlich und freundlich. Die Arbeit ist wirklich hervorragend. Sehr zu empfehlen.'],['Kosta Ruzic','KR','Ich war mit meinem Salonbesuch rundum zufrieden. Haarschnitt und Bartpflege wurden sehr sorgfältig und genau nach meinen Wünschen gemacht.'],["Radhwan Ali", "RA", "Der Friseur ist professionell, freundlich und nimmt sich viel Zeit für jeden Kunden Ich komme auf jeden Fall wieder! Vielen Dank"],["Horstek Joppe", "HJ", "War heute bei dem neuen Inhaber zum Herren Haarschnitt. Bin mehr als zufrieden. Nette Atmosphäre und sehr professioneller Haarschnitt. Komme gerne wieder."]];document.getElementById('review-track').innerHTML=[0,1].map(copy=>reviews.map(r=>`<article class="review-card" ${copy?'aria-hidden="true"':''}><div><span class="stars" aria-label="5 von 5 Sternen">★★★★★</span><blockquote>„${r[2]}“</blockquote></div><div class="review-person"><span class="avatar">${r[1]}</span><div><b>${r[0]}</b><small>Google-Bewertung</small></div></div></article>`).join('')).join('');const pause=document.getElementById('pause-reviews');pause.addEventListener('click',()=>{const paused=document.querySelector('.marquee').classList.toggle('paused');pause.setAttribute('aria-pressed',paused);pause.setAttribute('aria-label',paused?'Bewertungsanimation fortsetzen':'Bewertungsanimation pausieren');pause.textContent=paused?'▶ Fortsetzen':'Ⅱ Pause'});
